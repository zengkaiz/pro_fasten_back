...
<template>
  <section class="g-news-layout">
    <div class="body">
      <div class="body-top">
        <div class="btn-area">
          <el-button type="primary" size="mini" @click="addNews()" class="add-btn">添加</el-button>
        </div>
      </div>
      <tableMode :message="message">
        ...
        <!-- <template slot="content" slot-scope="prop">
          <div v-html="prop.scope.row.content"></div>
        </template>-->
        <template slot="operate" slot-scope="prop">
          <div class="btn-icon">
            <span class="el-icon-edit-outline" @click="editNews(prop.scope.row.id)"></span>
          </div>
        </template>
      </tableMode>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 编辑
    editNews(id) {
      this.$router.push({ name: 'newsEdit', query: { id: id } })
    },
    // 改变启用状态
        ...
        .then((data) => {
          if (data) {
            ...
            data.datas.forEach((item) => {
              obj = { ...item.news, creator: { ...item.creator } }
